...
def insertDB(values, count):
    ...
    # SQL 插入语句
    sql = "INSERT INTO review(business_id, text, date) \
           VALUES (%s,%s,%s)"
...
f = open('/home/g19tka20/Downloads/yelp_dataset/xi_review_without_hack_date.txt', 'r')
line = f.readline()
while line:
    count += 1
    j = json.loads(line)
    ...
    val = ((j['business_id'], j['text'], j['date']), )
    #        ('Bruse', 'Jerry', 30, 'F', 3000),
    #        ('Lee', 'Tomcat', 40, 'M', 4000),
    #        ('zhang', 'san', 18, 'M', 1500))
...
f = open('/home/g19tka20/Downloads/yelp_dataset/review_error.tx','w')
for er in error:
    f.write(er+"\r\n")
